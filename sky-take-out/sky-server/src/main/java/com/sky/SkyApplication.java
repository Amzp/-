...
/* @SpringBootApplication 是一个注解，用于标识一个主程序类（Main class）是Spring Boot应用程序的入口点和配置类。
    ...
    通常情况下，@SpringBootApplication 注解会放在主程序类（Main class）上。当应用程序被启动时，Spring Boot会扫描注解所在的包及其子包中的组件，并根据需要自动配置Spring应用程序上下文。
...
public class SkyApplication {
    public static void main(String[] args) {
        ...
        log.info("启动SkyApplication...");
        SpringApplication.run(SkyApplication.class, args);
        log.info("成功启动SkyApplication...");
    }
}
